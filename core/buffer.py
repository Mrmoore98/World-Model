...
class PPORolloutStorage:
    def __init__(self, num_steps, num_processes, obs_shape, act_dim, device,
        ...
        self.hidden = zeros(num_steps, num_processes, 256*2)
    ...
    def compute_returns(self, next_value, gamma):
        if self.gae:
            ...
            advantages = torch.zeros_like(self.returns)
            deltas = torch.zeros_like(self.returns)
            gae = 0
            ...
            for step in reversed(range(self.rewards.size(0))):
                ...
                deltas[step] = self.rewards[step] + gamma * self.value_preds[step+1] * self.masks[step+1] - self.value_preds[step]*self.masks[step]
                gae = deltas[step] + gamma * self.gae_lambda * gae * self.masks[step+1]
